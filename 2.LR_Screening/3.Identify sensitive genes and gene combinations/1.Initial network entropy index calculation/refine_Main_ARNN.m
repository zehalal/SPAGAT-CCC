...
Accurate_predictions=0;
    ...
    if length(real_y) == length(traindata_y)
       ...
       INPUT_trainlength
        ...
        % union_predict_y_ARNN_gpu = gather(union_predict_y_ARNN); % Ensure to use GPU arrays
        % myreal_gpu = gather(myreal);
        % % Calculate MSE manually on GPU
        % mse_gpu = mean((union_predict_y_ARNN_gpu - myreal_gpu).^2, 'all');
        % % Calculate RMSE
        % RMSE = sqrt(mse_gpu);
